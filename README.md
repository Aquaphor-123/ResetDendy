...
Данный проект реализует сброс для приставок Dendy/Famicom/NES при нажатии определённой комбинации кнопок на джойстике.
...
(ВНИМАНИЕ! После программирования Fuse bit RSTDISBL перепрошить микроконтроллер можно будет только при подаче 12 В
...
The project is To reset the Dendy (Famicom/NES) by joystick.

This project resets for Dendy/Famicom/NES consoles when you pressed a certain combination of buttons on the joystick.

The project uses a minimum number of elements:
1) SOT23 breadboard;
...
After flashing, you need set Fuse bit RSTDISBL.
ATTENTION! After setting Fuse bit RSTDISBL repeat of re-flashing the microcontroller is possible only possible by applying 12V to the RESET pin of the microcontroller.
